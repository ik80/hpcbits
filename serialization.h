#ifndef DYNA_SERIALIZATION_H
#define DYNA_SERIALIZATION_H
...
// TODO: or just add static hash definition/calculation to existing SERIALIZATION_FRIENDS
// TODO: or just use https://github.com/Manu343726/ctti
#ifndef DYNA_SERIALIZATION_PACKET_HEADER_TYPE_OFFSET // When serializing you'll get set of buffers with this much reserved
#define DYNA_SERIALIZATION_PACKET_HEADER_TYPE_OFFSET 14 // This MUST MATCH the (fixed) size of your headers on the wire
#endif

#ifndef DYNA_SERIALIZATION_PACKET_HEADER_RESERVED // When serializing you'll get set of buffers with this much reserved
...
// Goal for this was fastest possible serialization that also doesnt look too ugly
// It has two modes (datagram/stream oriented)
//
// Datagram mode uses scatter gather operation during de/serialization
// When serializing, boundaryCounter is interleaved with fragment data in teh big TX buffer
// while simultaneously accumulating iovecs for later use in sendmmsg
// When deserializing from multiple iovecs, boudnary counter is at the beginning of each iovec buffer
//
...
struct SerializationContext
{
    ...
    inline char* FragmentBuffer() const
    {
        return &(buffers.back()->first[offset]);
    }
    ...
    inline bool EnoughSpaceFor(uint32_t size) const
    {
        return DYNA_SERIALIZATION_FRAGMENT_SIZE - offset >= size;
    }
    inline uint32_t SpaceLeftOnFragment() const 
    {
        return DYNA_SERIALIZATION_FRAGMENT_SIZE - offset;
    }
...
#define SERIALIZATION_FORWARDS(TYPENAME) \
    template <> void Serialize<TYPENAME>(TYPENAME & message, SerializationContext* pContext); \
    template <> void DeSerialize<TYPENAME>(TYPENAME & message, SerializationContext* pContext); \
    template <> void GetSerializeableLength<TYPENAME>(TYPENAME & message,  uint64_t & messageSize); \
    template <> void SerializeBuffer<TYPENAME>(TYPENAME & message, char * & pBuffer, uint64_t & messageSize); \
    template <> void DeSerializeBuffer<TYPENAME>(TYPENAME & message, char * & pBuffer);
...
// macro to force templated functions instantination, this was needed by some old version of gcc in some case, tests compile and run fine now without
#define SERIALIZATION_FORCE_INSTANTINATION(TYPENAME) void DummySerializationInstantination##TYPENAME() { char * pBuffer = nullptr; TYPENAME dummyObj; uint64_t dummySize; SerializationContext* pSerializationContext= nullptr; Serialize(dummyObj,pSerializationContext); DeSerialize(dummyObj, pSerializationContext); GetSerializeableLength(dummyObj, dummySize); SerializeBuffer(dummyObj,pBuffer, dummySize); DeSerializeBuffer(dummyObj, pBuffer);}

// Basic templated versions
template<typename S> void Serialize(S & message, SerializationContext* pContext)
{
    static_assert(std::is_pod<S>()); // non pod structure missing serialization methods specializations
    static_assert(sizeof(S) + sizeof(uint32_t) + DYNA_SERIALIZATION_PACKET_HEADER_RESERVED <= DYNA_SERIALIZATION_FRAGMENT_SIZE); // pod structure + boudnaryCounter size dont fit into UDP packet
    if (!pContext->EnoughSpaceFor(sizeof(S))) 
...
template<typename S> void DeSerialize(S & message,  SerializationContext* pContext) //pContext should have offset == 0, boundaryCounter == 0 and have iovecs ready
{
    static_assert(std::is_pod<S>()); // non pod structure missing serialization methods specializations
...
template<typename S> void GetSerializeableLength(S & message, uint64_t & messageSize)
{
    static_assert(std::is_pod<S>()); // non pod structure missing serialization methods specializations
    messageSize += sizeof(S);
}
template <typename S> void SerializeBuffer(S & message, char * & pBuffer, uint64_t & messageSize)
{
    *(reinterpret_cast<S*>(pBuffer)) = message;
    pBuffer += sizeof(S);
    messageSize += sizeof(S);
}
template <typename S> void DeSerializeBuffer(S & message, char * & pBuffer)
{
...
// std::vector
template<typename S> void Serialize(std::vector<S> & messageVector, SerializationContext* pContext)
{
    ...
    { 
        // serialize stuff in chunks with size slightly less than DYNA_SERIALIZATION_FRAGMENT_SIZE and handl it with few memcopy calls
        // layout:
    ...
    else 
    {
        ...
        for (typename std::vector<S>::iterator it = messageVector.begin(); it != messageVector.end(); ++it)
            Serialize(*it, pContext);
    }
}
template<typename S> void DeSerialize(std::vector<S> & messageVector,  SerializationContext* pContext)
{
    if (std::is_pod<S>::value == true && (sizeof(S) + sizeof(uint32_t) + DYNA_SERIALIZATION_PACKET_HEADER_RESERVED <= DYNA_SERIALIZATION_FRAGMENT_SIZE)) 
    { 
    ...
    else 
    {
        ...
        for (uint64_t i = 0; i < vectorSize; ++i)
            DeSerialize(messageVector[i], pContext);
    }
}
template<typename S> void GetSerializeableLength(std::vector<S> & messageVector, uint64_t & messageSize)
{
    // TODO: HERE, if ((std::is_pod<S>::value == true) && (sizeof(S) + sizeof(uint32_t) < DYNA_SERIALIZATION_FRAGMENT_SIZE)) ,
    // serialize stuff in chunks with size slightly less than DYNA_SERIALIZATION_FRAGMENT_SIZE and handl it with few memcopy calls
    messageSize += sizeof(uint64_t);
    for (uint64_t i = 0; i < messageVector.size(); ++i)
        GetSerializeableLength(messageVector[i], messageSize);
}
template < typename S > void SerializeBuffer(std::vector<S> & messageVector, char * & pBuffer, uint64_t & messageSize)
{
...
template < typename S > void DeSerializeBuffer(std::vector<S> & messageVector, char * & pBuffer)
{
    ...
    else 
    {
        ...
        messageVector.resize(vectorSize);
        for (uint64_t i = 0; i < vectorSize; ++i)
            DeSerializeBuffer(messageVector[i], pBuffer);
    }
}

// std::array
// TODO: fix this shit
template<typename S, std::size_t N> void Serialize(std::array<S,N> & messageArray, SerializationContext* pContext)
{
    if (std::is_pod<S>::value == true && (sizeof(S) + sizeof(uint32_t) + DYNA_SERIALIZATION_PACKET_HEADER_RESERVED <= DYNA_SERIALIZATION_FRAGMENT_SIZE)) 
    { 
        // serialize stuff in chunks with size slightly less than DYNA_SERIALIZATION_FRAGMENT_SIZE and handl it with few memcopy calls
    ...
    {
        for (typename std::array<S,N>::iterator it = messageArray.begin(); it != messageArray.end(); ++it)
            Serialize(*it, pContext);
    }
}
template<typename S, std::size_t N> void DeSerialize(std::array<S,N> & messageArray,  SerializationContext* pContext)
{
    if (std::is_pod<S>::value == true && (sizeof(S) + sizeof(uint32_t) + DYNA_SERIALIZATION_PACKET_HEADER_RESERVED <= DYNA_SERIALIZATION_FRAGMENT_SIZE)) 
    { 
        size_t arraySize = N;
    ...
    {
        for (typename std::array<S,N>::iterator it = messageArray.begin(); it != messageArray.end(); ++it)
            DeSerialize(*it, pContext);
    }
}
template<typename S, std::size_t N> void GetSerializeableLength(std::array<S,N> & messageArray, uint64_t & messageSize)
{
    if (std::is_pod<S>::value == true && (sizeof(S) + sizeof(uint32_t) + DYNA_SERIALIZATION_PACKET_HEADER_RESERVED <= DYNA_SERIALIZATION_FRAGMENT_SIZE)) 
    { 
        ...
        for (uint64_t i = 0; i < N; ++i)
            GetSerializeableLength(messageArray[i], messageSize);
}
template <typename S, std::size_t N> void SerializeBuffer(std::array<S,N> & messageArray, char * & pBuffer, uint64_t & messageSize)
{
    if (std::is_pod<S>::value == true) 
    {
...
template <typename S, std::size_t N> void DeSerializeBuffer(std::array<S,N> & messageArray, char * & pBuffer)
{
    // let users clear their own structures
    if (std::is_pod<S>::value == true) 
    {
    ...
    {
        for (typename std::array<S,N>::iterator it = messageArray.begin(); it != messageArray.end(); ++it)
            DeSerializeBuffer(*it, pBuffer);
    }
}

// fixed size array
template<typename S, int N> void Serialize(S (&messageArray)[N], SerializationContext* pContext)
{
    if (std::is_pod<S>::value == true && (sizeof(S) + sizeof(uint32_t) + DYNA_SERIALIZATION_PACKET_HEADER_RESERVED <= DYNA_SERIALIZATION_FRAGMENT_SIZE)) 
    { 
        // serialize stuff in chunks with size slightly less than DYNA_SERIALIZATION_FRAGMENT_SIZE and handl it with few memcopy calls
...
template<typename S, int N> void DeSerialize(S (&messageArray)[N],  SerializationContext* pContext)
{
    if (std::is_pod<S>::value == true && (sizeof(S) + sizeof(uint32_t) + DYNA_SERIALIZATION_PACKET_HEADER_RESERVED <= DYNA_SERIALIZATION_FRAGMENT_SIZE)) 
    { 
        size_t arraySize = N;
...
template<typename S, int N> void GetSerializeableLength(S (&messageArray)[N], uint64_t & messageSize)
{
    if (std::is_pod<S>::value == true && (sizeof(S) + sizeof(uint32_t) + DYNA_SERIALIZATION_PACKET_HEADER_RESERVED <= DYNA_SERIALIZATION_FRAGMENT_SIZE)) 
    { 
        messageSize += N*sizeof(S);
...
template <typename S, int N> void SerializeBuffer(S (&messageArray)[N], char * & pBuffer, uint64_t & messageSize)
{
    if (std::is_pod<S>::value == true) 
    {
        ...
        for (uint64_t i =0; i < N; ++i)
            SerializeBuffer(messageArray[i], pBuffer, messageSize);
}
template <typename S, int N> void DeSerializeBuffer(S (&messageArray)[N], char * & pBuffer)
{
    if (std::is_pod<S>::value == true) 
    {
        ...
        for (uint64_t i = 0; i < N; ++i)
            DeSerializeBuffer(messageArray[i], pBuffer);
}

// dynamically allocated array
template<typename S> void Serialize(S * messageArray, uint64_t & numElements, SerializationContext* pContext)
{
    // TODO: HERE, if ((std::is_pod<S>::value == true) && (sizeof(S) + sizeof(uint32_t) < DYNA_SERIALIZATION_FRAGMENT_SIZE)) ,
    // serialize stuff in chunks with size slightly less than DYNA_SERIALIZATION_FRAGMENT_SIZE and handl it with few memcopy calls
...
template<typename S> void DeSerialize(S * messageArray, uint64_t & numElements,  SerializationContext* pContext)
{
    // TODO: HERE, if ((std::is_pod<S>::value == true) && (sizeof(S) + sizeof(uint32_t) < DYNA_SERIALIZATION_FRAGMENT_SIZE)) ,
    // serialize stuff in chunks with size slightly less than DYNA_SERIALIZATION_FRAGMENT_SIZE and handl it with few memcopy calls
...
template<typename S> void GetSerializeableLength(S * messageArray, uint64_t & numElements, uint64_t & messageSize)
{
    // TODO: HERE, if ((std::is_pod<S>::value == true) && (sizeof(S) + sizeof(uint32_t) < DYNA_SERIALIZATION_FRAGMENT_SIZE)) ,
    // serialize stuff in chunks with size slightly less than DYNA_SERIALIZATION_FRAGMENT_SIZE and handl it with few memcopy calls
    messageSize += sizeof(uint64_t) + numElements*sizeof(S);
}
template < typename S > void SerializeBuffer(S * messageArray, uint64_t & numElements, char * & pBuffer, uint64_t & messageSize)
{
    if (std::is_pod<S>::value == true) 
    {
...
template < typename S > void DeSerializeBuffer(S * messageArray, uint64_t & numElements, char * & pBuffer)
{
    if (std::is_pod<S>::value == true) 
    {
        // let users clear their own structures
...
// std::pair
template<typename SK, typename SV> void Serialize(std::pair<SK, SV> & message, SerializationContext* pContext)
{
    Serialize(message.first, pContext);
    Serialize(message.second, pContext);
}
template<typename SK, typename SV> void DeSerialize(std::pair<SK, SV> & message,  SerializationContext* pContext)
{
    DeSerialize(message.first, pContext);
    DeSerialize(message.second, pContext);
}
template<typename SK, typename SV> void GetSerializeableLength(std::pair<SK, SV> & message, uint64_t & messageSize)
{
    GetSerializeableLength(message.first, messageSize);
    GetSerializeableLength(message.second, messageSize);
}
template < typename SK, typename SV > void SerializeBuffer(std::pair<SK, SV> & message, char * & pBuffer, uint64_t & messageSize)
{
    SerializeBuffer(message.first, pBuffer, messageSize);
    SerializeBuffer(message.second, pBuffer, messageSize);
}
template < typename SK, typename SV > void DeSerializeBuffer(std::pair<SK, SV> & message, char * & pBuffer)
{
...
// std::list
template<typename S> void Serialize(std::list<S> & messageList, SerializationContext* pContext)
{
...
template<typename S> void DeSerialize(std::list<S> & messageList,  SerializationContext* pContext)
{
    uint64_t listSize = 0;
...
template<typename S> void GetSerializeableLength(std::list<S> & messageList, uint64_t & messageSize)
{
    messageSize += sizeof(uint64_t);
    for (typename std::list<S>::iterator it = messageList.begin(); it != messageList.end(); ++it)
        GetSerializeableLength(*it, messageSize);
}
template < typename S > void SerializeBuffer(std::list<S> & messageList, char * & pBuffer, uint64_t & messageSize)
{
    uint64_t listSize = 0;
...
template < typename S > void DeSerializeBuffer(std::list<S> & messageList, char * & pBuffer)
{
    // let users clear their own structures
    //messageList.clear();
    uint64_t listSize = 0;
...
template<typename S> void Serialize(std::deque<S> & messageDeque, SerializationContext* pContext)
{
...
template<typename S> void DeSerialize(std::deque<S> & messageDeque,  SerializationContext* pContext)
{
    uint64_t listSize = 0;
    DeSerialize(listSize, pContext);
    for (uint64_t i = 0; i < listSize; ++i)
...
template<typename S> void GetSerializeableLength(std::deque<S> & messageDeque, uint64_t & messageSize)
{
    messageSize += sizeof(uint64_t);
    for (typename std::deque<S>::iterator it = messageDeque.begin(); it != messageDeque.end(); ++it)
        GetSerializeableLength(*it, messageSize);
}
template < typename S > void SerializeBuffer(std::deque<S> & messageDeque, char * & pBuffer, uint64_t & messageSize)
{
    uint64_t listSize = 0;
    char * listSizePos = pBuffer;
    pBuffer += sizeof(uint64_t);
...
template < typename S > void DeSerializeBuffer(std::deque<S> & messageDeque, char * & pBuffer)
{
    // let users clear their own structures
    //messageDeque.clear();
    uint64_t listSize = 0;
...
template<typename SK, typename SV> void Serialize(std::map<SK, SV> & messageMap, SerializationContext* pContext)
{
...
template<typename SK, typename SV> void DeSerialize(std::map<SK, SV> & messageMap,  SerializationContext* pContext)
{
    uint64_t mapSize = 0;
...
template<typename SK, typename SV> void GetSerializeableLength(std::map<SK, SV> & messageMap, uint64_t & messageSize)
{
    messageSize += sizeof(uint64_t);
    for (typename std::map<SK, SV>::iterator it = messageMap.begin(); it != messageMap.end(); ++it)
    {
...
template < typename SK, typename SV > void SerializeBuffer(std::map<SK, SV> & messageMap, char * & pBuffer, uint64_t & messageSize)
{
    uint64_t mapSize = 0;
...
template < typename SK, typename SV > void DeSerializeBuffer(std::map<SK, SV> & messageMap, char * & pBuffer)
{
    // let users clear their own structures
    //messageMap.clear();
    uint64_t mapSize = 0;
...
// std::unordered_map
template<typename SK, typename SV> void Serialize(std::unordered_map<SK, SV> & messageMap, SerializationContext* pContext)
{
    uint64_t mapSize = messageMap.size();
    Serialize(mapSize, pContext);
...
template<typename SK, typename SV> void DeSerialize(std::unordered_map<SK, SV> & messageMap,  SerializationContext* pContext)
{
    uint64_t mapSize = 0;
    DeSerialize(mapSize, pContext);
    for (uint64_t i = 0; i < mapSize; ++i)
...
template<typename SK, typename SV> void GetSerializeableLength(std::unordered_map<SK, SV> & messageMap, uint64_t & messageSize)
{
    messageSize += sizeof(uint64_t);
    for (typename std::unordered_map<SK, SV>::iterator it = messageMap.begin(); it != messageMap.end(); ++it)
    {
...
template < typename SK, typename SV > void SerializeBuffer(std::unordered_map<SK, SV> & messageMap, char * & pBuffer, uint64_t & messageSize)
{
    uint64_t mapSize = 0;
    char * mapSizePos = pBuffer;
    pBuffer += sizeof(uint64_t);
...
template < typename SK, typename SV > void DeSerializeBuffer(std::unordered_map<SK, SV> & messageMap, char * & pBuffer)
{
    // let users clear their own structures
    //messageMap.clear();
    uint64_t mapSize = 0;
...
template<> void SerializeBuffer<C>(C & message, char * & pBuffer, uint64_t & messageSize)
{
...
{
    return lhs.vectorB == rhs.vectorB && lhs.moarInts == rhs.moarInts && lhs.listA == rhs.listA && lhs.mapVectorBListA == rhs.mapVectorBListA;
}
bool operator!=(const C& lhs, const C& rhs) 
{
    return !(lhs == rhs);
...
void test() 
{
    ...
    SerializationContext * pSerializationContextOut = new SerializationContext();
    {
        a.a = 1;
        a.b = 2;
        a.c = 3;
    ...
    std::reverse(pSerializationContextIn->buffers.begin(), pSerializationContextIn->buffers.end());
    delete pSerializationContextOut;
